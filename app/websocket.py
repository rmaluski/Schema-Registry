...
class WebSocketManager:
    ...
    async def connect(
        self, websocket: WebSocket, channel: str, client_info: Dict[str, Any] = None
    ):
    ...
    async def broadcast_schema_update(
        self, schema_id: str, version: str, action: str, schema_data: Dict[str, Any]
    ):
    ...
    async def broadcast_compatibility_alert(
        self,
        schema_id: str,
        old_version: str,
        new_version: str,
        ...
        logger.info(
            f"Broadcasted compatibility alert: {schema_id} {old_version} -> {new_version}"
        )
...
class WebSocketHandler:
    ...
    @staticmethod
    async def handle_schema_updates(
        websocket: WebSocket, client_info: Dict[str, Any] = None
    ):
                ...
                if message.get("type") == "subscribe":
                    ...
                    if schema_id:
                        ...
                        await websocket_manager.send_personal_message(
                            websocket,
                            {"type": "subscription_confirmed", "schema_id": schema_id},
                        )
    ...
    @staticmethod
    async def handle_compatibility_alerts(
        websocket: WebSocket, client_info: Dict[str, Any] = None
    ):
    ...
    @staticmethod
    async def handle_system_events(
        websocket: WebSocket, client_info: Dict[str, Any] = None
    ):
...
async def on_schema_created(schema_id: str, version: str, schema_data: Dict[str, Any]):
    ...
    await websocket_manager.broadcast_schema_update(
        schema_id, version, "created", schema_data
    )
...
async def on_schema_updated(schema_id: str, version: str, schema_data: Dict[str, Any]):
    ...
    await websocket_manager.broadcast_schema_update(
        schema_id, version, "updated", schema_data
    )
...
async def on_compatibility_check(
    schema_id: str, old_version: str, new_version: str, breaking_changes: List[str]
):
    """Hook called when compatibility is checked."""
    if breaking_changes:
        await websocket_manager.broadcast_compatibility_alert(
            schema_id, old_version, new_version, breaking_changes
        )
