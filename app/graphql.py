...
class GraphQLSchemaRegistry:
    ...
    def _build_schema(self) -> GraphQLSchema:
        ...
        SchemaType = GraphQLObjectType(
            ...
            fields={
                ...
                "properties": GraphQLField(
                    GraphQLNonNull(GraphQLString), resolve=self._resolve_properties
                ),
                "required_fields": GraphQLField(
                    GraphQLList(GraphQLString), resolve=self._resolve_required_fields
                ),
                "field_count": GraphQLField(
                    GraphQLInt, resolve=self._resolve_field_count
                ),
                "has_arrow_schema": GraphQLField(
                    GraphQLBoolean, resolve=self._resolve_has_arrow
                ),
                "created_at": GraphQLField(
                    GraphQLString, resolve=self._resolve_created_at
                ),
                "updated_at": GraphQLField(
                    GraphQLString, resolve=self._resolve_updated_at
                ),
            },
        )
        ...
        SchemaVersionType = GraphQLObjectType(
            ...
            fields={
                "version": GraphQLField(GraphQLNonNull(GraphQLString)),
                "schema": GraphQLField(
                    SchemaType, resolve=self._resolve_schema_version
                ),
                "is_latest": GraphQLField(
                    GraphQLBoolean, resolve=self._resolve_is_latest
                ),
                "is_compatible_with": GraphQLField(
                    GraphQLBoolean,
                    args={"target_version": GraphQLNonNull(GraphQLString)},
                    resolve=self._resolve_compatibility,
                ),
            },
        )
        ...
        QueryType = GraphQLObjectType(
            ...
            fields={
                "schemas": GraphQLField(
                    GraphQLList(SchemaType), resolve=self._resolve_schemas
                ),
                "schema": GraphQLField(
                    SchemaType,
                    args={
                    ...
                    resolve=self._resolve_schema,
                ),
                "schemaVersions": GraphQLField(
                    GraphQLList(SchemaVersionType),
                    args={"id": GraphQLNonNull(GraphQLString)},
                    resolve=self._resolve_schema_versions,
                ),
                "searchSchemas": GraphQLField(
                    GraphQLList(SchemaType),
                    args={"query": GraphQLNonNull(GraphQLString), "limit": GraphQLInt},
                    resolve=self._resolve_search_schemas,
                ),
                "compatibleSchemas": GraphQLField(
                    ...
                    resolve=self._resolve_compatible_schemas,
                ),
                "schemaStats": GraphQLField(
                    GraphQLString,
                    args={"id": GraphQLNonNull(GraphQLString)},
                    resolve=self._resolve_schema_stats,
                ),
            },
        )
    ...
    def _resolve_schema(
        self, info, id: str, version: Optional[str] = None
    ...
    def _resolve_search_schemas(
        self, info, query: str, limit: Optional[int] = 10
    ) -> List[Dict[str, Any]]:
    ...
    def _resolve_compatible_schemas(
        self, info, schema_id: str, version: str
    ) -> List[Dict[str, Any]]:
    ...
    def _resolve_schema_stats(self, info, id: str) -> str:
        ...
        try:
            ...
            stats = {
                ...
                "latest_version": latest_schema["version"] if latest_schema else None,
                "field_count": len(latest_schema.get("properties", {}))
                if latest_schema
                else 0,
                "has_arrow_schema": bool(latest_schema.get("arrow"))
                if latest_schema
                else False,
            }
