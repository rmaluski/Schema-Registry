...
class SchemaCache:
    ...
    @staticmethod
    def get_schema(schema_id: str, version: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Get schema from cache."""
        key = f"{schema_id}:{version or 'latest'}"
        return cache.get("schema", key)
    
    @staticmethod
    def set_schema(schema_id: str, version: Optional[str], schema_data: Dict[str, Any]) -> bool:
        """Set schema in cache."""
        key = f"{schema_id}:{version or 'latest'}"
