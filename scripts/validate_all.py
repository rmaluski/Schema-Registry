...
def validate_schema_file(file_path: Path) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        (
            ...
            error_msg,
            validation_errors,
        ) = SchemaValidator.validate_schema_document(schema_data)
        ...
        if schema_id != expected_id:
            errors.append(
                f"Schema ID '{schema_id}' doesn't match expected '{expected_id}'"
            )
            return False, errors
...
def main():
    ...
    for schema_file in schema_files:
        ...
        validation_results[schema_file.name] = {"valid": is_valid, "errors": errors}
